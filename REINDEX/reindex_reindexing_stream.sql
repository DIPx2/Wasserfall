...
-- DROP PROCEDURE IF EXISTS public."reindexing_stream"(double precision);

CREATE OR REPLACE PROCEDURE Public."reindexing_stream"(IN bloat_satio_search DOUBLE PRECISION DEFAULT 25) -- Входной параметр: пороговое значение в % для поиска "раздутых" индексов
    LANGUAGE 'plpgsql'
AS
$BODY$ -- VERSION 0-00-000
DECLARE
...
BEGIN
    ...
    BEGIN
        ...
        FOR server IN SELECT JSON_BUILD_OBJECT('Id_Conn', Pk_Id_Conn, 'port', Conn_Port, 'host', Conn_Host) AS server
                      FROM "Bloat_Index_Db_Connections"
            LOOP
                -- Цикл по всем базам данных, связанным с текущим сервером
                FOR database IN SELECT JSON_BUILD_OBJECT('Id_Db', Pk_Id_Db, 'Id_Conn', Fk_Pk_Id_Conn::TEXT, 'Scheme',
