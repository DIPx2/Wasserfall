...
DECLARE fts text;
BEGIN
    FOR fts IN (SELECT DISTINCT foreign_table_schema FROM information_schema.foreign_tables WHERE RIGHT(foreign_table_schema, 6) = 'master')
    LOOP
        BEGIN
            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = fts AND table_name = 'users') THEN
                EXECUTE FORMAT('CREATE TABLE %I.users_old AS SELECT * FROM %I.users', fts, fts);
            END IF;

            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = fts AND table_name = 'groups') THEN
                EXECUTE FORMAT('CREATE TABLE %I.groups_old AS SELECT * FROM %I.groups', fts, fts);
            END IF;

            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = fts AND table_name = 'user_groups') THEN
                EXECUTE FORMAT('CREATE TABLE %I.user_groups_old AS SELECT * FROM %I.user_groups', fts, fts);
            END IF;

            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = fts AND table_name = 'user_project') THEN
                EXECUTE FORMAT('CREATE TABLE %I.user_project_old AS SELECT * FROM %I.user_project', fts, fts);
            END IF;
        END;
    END LOOP;
END;
