...
BEGIN
    ...
    SELECT 'y_connect'                                 AS conn_name,
           ...
           REGEXP_REPLACE( FORMAT('%s--%s', Robohub.Robo_Reference."Servers".Conn_Host, Robohub.Robo_Reference."Servers".Conn_Port), '\.maxbit\.private', '', 'g' ) || '--' || TO_CHAR(DATE_TRUNC('hour', start_time AT TIME ZONE 'UTC'), 'YYYY-MM-DD--HH24-MI') || '.html'
                                                       AS compl_name,
