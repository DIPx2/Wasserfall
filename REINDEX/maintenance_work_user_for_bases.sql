    ...
    BEGIN
        ...
        BEGIN
            FOR server IN SELECT JSON_BUILD_OBJECT('Id_Conn', Pk_Id_Conn, 'port', Conn_Port, 'host', Conn_Host) AS server FROM "Bloat_Index_Db_Connections"
                LOOP
                    FOR database IN SELECT JSON_BUILD_OBJECT('Id_Db', Pk_Id_Db, 'Id_Conn', Fk_Pk_Id_Conn::TEXT, 'Scheme', Db_Scheme, 'Name', Db_Name) AS database FROM "Bloat_Index_Db_Connections_Details" WHERE Fk_Pk_Id_Conn = (server ->> 'Id_Conn')::INTEGER
                        LOOP
