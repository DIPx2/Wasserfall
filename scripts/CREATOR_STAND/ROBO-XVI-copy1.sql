...
BEGIN
    ...
    FOREACH db_name IN ARRAY arr_stage LOOP
        ...
        conn_str = format( 'dbname=%s user=robo_sudo password=%s host=localhost port=15434', db_name, '%dFgH8!zX4&kLmT2' );
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING 'ERROR %: %', db_name, SQLERRM;
        END;
...
BEGIN
    FOREACH k IN ARRAY arr
    LOOP
        conn_name = format('conn_%s',  k);
        conn_str = format( 'dbname=%s user=robo_sudo password=%s host=localhost port=15434', k, '%dFgH8!zX4&kLmT2' );
